...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        """
        B_, N, C = x.shape  # B_: num_windows*B, N: window_size*window_size, C: dim
        qkv = (
            self.qkv(x)
...
class BasicLayer(nn.Module):
    ...
    def forward(
                ...
                for i in range(num_blocks):
                    if i == 0:
                        ...
                        prompt_emb = deep_prompt_embd[
                            :, :, i, ...
                        ]  # all experts at this block # [B, num_experts, prompt_len, dim]
                        ...
                        x, imp_loss, dyn_prompt = self.blocks[i].forward_mm_moe(
                            x,
                            static_prompt[:, i, ...],
                        ...
                        dyn_prompts.append(dyn_prompt)
                    else:
                        prompt_emb = deep_prompt_embd[:, i, ...]  # [B, prompt_len, dim]
                        if self.use_instruct:
        ...
        if len(imp_losses) > 0:
            if len(dyn_prompts) > 0:
                layer_dyn_prompt = torch.stack(dyn_prompts, dim=0).mean(dim=0)

            else:
                layer_dyn_prompt = None
            return x, imp_losses, layer_dyn_prompt
        return x
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        patches_resolution = [
            img_size[0] // patch_size[0],  # 224//4 = 56
            img_size[1] // patch_size[1],  # 224//4 = 56
        ]
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        f = self.norm(x)
        cls_ = self.avgpool(f.transpose(1, 2))
...
class PromptedSwinTransformer(SwinTransformer):
        ...
        **kwargs,
        ...
        if True:
            ...
            self.prompt_embeddings = nn.Parameter(
                torch.zeros(1, self.num_prompts, embed_dim)  # [1, num_prompts, embed_dim]
            ...
            if True:  # set false to avoid extra param for ablation 消融实验的时候可以开关这些投影层
                ...
                # Feature experts
                self.num_feature_experts = 3

                self.feature_experts = nn.ModuleList([
                    FeatureExpert(self.num_features)
                    for _ in range(self.num_feature_experts)
                ])

                self.feature_gate = FeatureGate(
                    self.num_features,
                    self.num_feature_experts
                )

                self.prompt_expert_scale = nn.Parameter(torch.tensor(0.1))
    ...
    def incorporate_prompt(self, x):
        ...
        prompt_embd = self.prompt_dropout(self.prompt_embeddings.expand(B, -1, -1))  # [1, num_prompts, dim] → [B, num_prompts, dim]
    ...
    def forward_features_instruct(self, x, y, return_internal=False):
        ...
        deep_prompt_embds = [
            self.deep_prompt_embeddings_0[0].expand(B, -1, -1, -1),  # [B, 2, num_tokens, 96]
            self.deep_prompt_embeddings_1[0].expand(B, -1, -1, -1),  # [B, 2, num_tokens, 192]
            self.deep_prompt_embeddings_2[0].expand(B, -1, -1, -1),  # [B, 6, num_tokens, 384]
            self.deep_prompt_embeddings_3[0].expand(B, -1, -1, -1),  # [B, 2, num_tokens, 768]
        ]
    ...
    def forward_features_instruct_moe(self, x, y, route_score, return_internal=False):
        ...
        if len(route_score.shape) == 2:
            route_per_layer = False  # [B, n_expert]
        elif len(route_score.shape) == 3:
            route_per_layer = True  # [B, n_layer, n_expert]
        ...
        othro_loss = []
        for i in range(len(self.layers)):  # i = 0,1,2,3
            if route_per_layer:
    ...
    def forward_features_attn_moe(self, x, y, route_score):
        ...
        for i in range(len(all_prompt_experts)):
            ...
            static_prompt_expanded = (
                static_prompt[i].unsqueeze(1)  # [B, 1, depth, num_tokens, dim]
                .expand(-1, self.moe_n_experts, -1, -1, -1)  # [B, k, depth, num_tokens, dim]
            )
    ...
    def forward_features_instruct_multimodal_moe(self, x, y):
        ...
        # if self.prompt_config.LOCATION == "prepend" and self.prompt_config.DEEP:
        y_ins = self.mm_prompt_instruction_proj(y)  # [B, 768] → [B, 384]
        ...
        extra_out = {}
        all_layer_imp_loss = []
        all_layer_dynamic_prompts = []
        # here, default to route per layer
        for i in range(len(self.layers)):
            x, imp_losses, layer_dynamic_prompt = self.layers[i](
                x,
            ...
            all_layer_dynamic_prompts.append(layer_dynamic_prompt)

        extra_out["importance_loss"] = torch.mean(torch.stack(all_layer_imp_loss))
        extra_out["dynamic_prompts"] = all_layer_dynamic_prompts

        f = self.norm(x)  # B L C

        B, L, C = f.shape
        num_prompts = self.layers[0].num_prompts

        prompt_features = f[:, :num_prompts, :]
        img_features = f[:, num_prompts:, :]

        L_img = img_features.shape[1]
        H = W = int(math.sqrt(L_img))
        assert H * W == L_img, f"Image feature length {L_img} cannot be reshaped to square: {H}*{W}={H * W}"

        feat_map = img_features.transpose(1, 2).view(B, C, H, W)  # [B, C, H, W]
        feat_map_res = feat_map

        expert_outputs = []
        for expert in self.feature_experts:
            expert_outputs.append(expert(feat_map))

        feat_map_expert = self.feature_gate(feat_map, expert_outputs)
        # residual
        feat_map = feat_map_res + self.prompt_expert_scale * feat_map_expert

        img_features = feat_map.flatten(2).transpose(1, 2)

        f = torch.cat([prompt_features, img_features], dim=1)
...
class PromptedPatchMerging(PatchMerging):
    ...
    def forward(self, x):
        ...
        if self.prompt_location == "prepend":
            # change input size
            prompt_emb = x[:, : self.num_prompts, :]  # [B, num_prompts, C]
            x = x[:, self.num_prompts :, :]  # [B, H*W, C]
            L = L - self.num_prompts
            prompt_emb = self.upsample_prompt(prompt_emb)  # [B, num_prompts, 4*C]
        ...
        # add the prompt back:
        if self.prompt_location == "prepend":
            x = torch.cat((prompt_emb, x), dim=1)  # [B, num_prompts + (H/2)*(W/2), 4*C]

        x = self.norm(x)
        x = self.reduction(x)  # Linear(4C → 2C)

        return x  # [B, num_prompts + (H/2)*(W/2), 2*C]
...
class PromptedSwinTransformerBlock(SwinTransformerBlock):
    ...
    def forward_mm_moe(
        ...
        moe_logits = moe_joint_embd @ self.mm_frozen_expert_key_embed  # B, k_expert
        temperature = 0.1
        ...
        sum_scores = torch.sum(moe_scores, dim=0)
        ...
        return self.forward(x), importance_loss, dynamic_prompt
...
def get_swin_encoder(
    ...
    if use_vpt:
        ...
        for name, param in model.named_parameters():
            if "prompt" in name or "feature_experts" in name or "feature_gate" in name:
