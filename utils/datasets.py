...
class ISIC2019Dataset(Dataset):
    def __init__(self, data_root, split="train", transforms=None):
        ...
        global_meta_file = os.path.join(self.data_root, "ISIC_2019_Training_Metadata.csv")
        global_meta_df = pd.read_csv(global_meta_file)

        self.global_age_median = global_meta_df["age_approx"].median()
        self.global_age_min = global_meta_df["age_approx"].min()
        self.global_age_max = global_meta_df["age_approx"].max()

        all_sex_categories = sorted(global_meta_df["sex"].dropna().str.lower().unique())
        self.expected_sex_cols = [f"sex_{c}" for c in all_sex_categories]

        all_site_categories = sorted(global_meta_df["anatom_site_general"].dropna().str.lower().unique())
        if "unknown" not in all_site_categories:
            all_site_categories.append("unknown")
        self.expected_site_cols = [f"site_{c}" for c in all_site_categories]

        self.site_dim = len(self.expected_site_cols)
        self.sex_dim = len(self.expected_sex_cols)

        labels_df = pd.read_csv(os.path.join(data_root, f"{split}_labels.csv"))
        meta_df = pd.read_csv(os.path.join(data_root, f"{split}_metadata.csv"))

        self.data_df = pd.merge(labels_df, meta_df, on="image")

        self.classes = ['MEL', 'NV', 'BCC', 'AK', 'BKL', 'DF', 'VASC', 'SCC']

        self.meta_tensors = self._process_metadata(self.data_df)

    def _process_metadata(self, df: pd.DataFrame):
        df_copy = df.copy()
        # ==== sex ====
        df_copy["sex"] = df_copy["sex"].fillna("male").str.lower()
        sex_dummies = pd.get_dummies(df_copy["sex"], prefix="sex")
        for col in self.expected_sex_cols:
            if col not in sex_dummies:
                sex_dummies[col] = 0
        sex_dummies = sex_dummies[self.expected_sex_cols].astype(np.float32)

        # ==== age ====
        df_copy["age_approx"] = df_copy["age_approx"].fillna(self.global_age_median)
        df_copy["age_norm"] = (df_copy["age_approx"] - self.global_age_min) / (
                    self.global_age_max - self.global_age_min)

        # ==== site ====
        df_copy["anatom_site_general"] = df_copy["anatom_site_general"].fillna("unknown").str.lower()
        site_dummies = pd.get_dummies(df_copy["anatom_site_general"], prefix="site")
        for col in self.expected_site_cols:
            if col not in site_dummies:
                site_dummies[col] = 0
        site_dummies = site_dummies[self.expected_site_cols].astype(np.float32)

        meta_matrix = pd.concat([df_copy[["age_norm"]], sex_dummies, site_dummies], axis=1)
        return torch.tensor(meta_matrix.values, dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(self.data_root, self.split, f"{img_name}.jpg")
        image = Image.open(img_path).convert("RGB")
        if self.transforms:
            image = self.transforms(image)

        meta_tensor = self.meta_tensors[idx]

        label_one_hot = row[self.classes].values.astype(np.float32)
        label_idx = np.argmax(label_one_hot)
        label = torch.LongTensor([label_idx])

        return meta_tensor, image, label
...
class HAM10000Dataset(Dataset):
    def __init__(self, data_root, split="train", transforms=None):
        ...
        csv_path = os.path.join(data_root, "ProcessedMeta", f"{split}.csv")
        self.data_df = pd.read_csv(csv_path)

        train_csv_path = os.path.join(data_root, "ProcessedMeta", "train.csv")
        train_df = pd.read_csv(train_csv_path)

        self.classes = sorted(train_df['dx'].unique().tolist())
        self.class_to_idx = {cls_name: i for i, cls_name in enumerate(self.classes)}
        self.num_classes = len(self.classes)

        self.features = ['dx_type', 'sex', 'localization']

        self.age_min = train_df['age'].min()
        self.age_max = train_df['age'].max()
        self.data_df['age_norm'] = (self.data_df['age'] - self.age_min) / (self.age_max - self.age_min + 1e-8)

        meta_tensors_list = [self.data_df[['age_norm']]]
        self.feature_dims = [1]

        for col in self.features:

            train_dummies = pd.get_dummies(train_df[col], prefix=col)
            expected_cols = train_dummies.columns.tolist()
            self.feature_dims.append(len(expected_cols))

            split_dummies = pd.get_dummies(self.data_df[col], prefix=col)
            for expected_col in expected_cols:
                if expected_col not in split_dummies:
                    split_dummies[expected_col] = 0

            split_dummies = split_dummies[expected_cols].astype(np.float32)
            meta_tensors_list.append(split_dummies)

        meta_matrix = pd.concat(meta_tensors_list, axis=1)
        self.meta_tensors = torch.tensor(meta_matrix.values, dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        ...
        label_str = row['dx']
        label = torch.LongTensor([self.class_to_idx[label_str]])
...
class JsonlDataset(Dataset):
    def __init__(self, data_root, split: str = "train", transforms=None):
        ...
        if "food" in data_root:
            self.data_type = "uni_label"  # 单标签分类
        elif "imdb" in data_root:
            self.data_type = "multi_label"  # 多标签分类
        elif "snli" in data_root:
            self.data_type = "visual_entailment"  # 视觉蕴含
...
def create_loaders(data_root, batch_size, num_workers, n_shot=0):
    ...
    if "isic2019" in data_root.lower():
        transforms_train = transforms.Compose(
            [
        ...
        trainset = ISIC2019Dataset(data_root, "train", transforms_train)
        validset = ISIC2019Dataset(data_root, "valid", transforms_val)
        testset = ISIC2019Dataset(data_root, "test", transforms_test)

    elif "ham10000" in data_root.lower():
        transforms_train = transforms.Compose([
            transforms.Resize((256, 256)),
            transforms.CenterCrop(224),
        ...
        trainset = HAM10000Dataset(data_root, "train", transforms_train)
        validset = HAM10000Dataset(data_root, "val", transforms_val)
        testset = HAM10000Dataset(data_root, "test", transforms_val)

    else:
        # Training and validation sets
        trainset = JsonlDataset(data_root, "train", transforms_train)
        validset = JsonlDataset(data_root, "dev", transforms_val)
